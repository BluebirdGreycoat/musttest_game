...
function armor.damage_type_disables_cloak(rstr)
	if rstr == "fleshy" or rstr == "arrow" or rstr == "boom"
			or rstr == "cracky" or rstr == "freeze" or rstr == "snappy" or rstr == "choppy"
			or rstr == "crush" then
		return true
	end
	return false
...
function armor.on_player_hp_change(player, hp_change, reason)
	...
	-- Simulate a 'stun' effect for heavy hits.
	if hp_change <= -(3*500) then
		pova.update_modifier(player, "physics", {sneak=false}, "damage.stun.1", {time=2})
	end
	if hp_change <= -(6*500) then
