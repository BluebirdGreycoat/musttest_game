...
for j, t in ipairs({
	...
	local func = _G["cent2_" .. t.tier]

	-- Read values from balancing table.
	local techname = "centrifuge_" .. t.tier
	...
	func.on_timer = function(pos, elapsed)
		...
		do
			if keeprunning then
				...
				machines.swap_node(pos, "cent2:" .. t.tier .. "_active")
				...
				meta:set_int("fueltotaltime", 0)
				meta:set_float("srctime", 0)
				machines.swap_node(pos, "cent2:" .. t.tier .. "_inactive")
	...
	func.on_blast = function(pos)
		...
		drops[#drops+1] = "cent2:" .. t.tier .. "_inactive"
...
if not cent2.run_once then
	for j, t in ipairs({
		...
		local SIDE_ANIMATED = {
			name = "centrifuge_" .. t.tier .. "_side_active.png",
			animation = {
		...
		local FRONT_ANIMATED = {
			image = "centrifuge_" .. t.tier .. "_front_active.png",
		...
		for k, v in ipairs({
			{name="inactive", light=0, tile="centrifuge_" .. t.tier .. "_front.png", side="centrifuge_" .. t.tier .. "_side.png"},
			{name="active", light=8, tile=FRONT_ANIMATED, side=SIDE_ANIMATED},
		}) do
			minetest.register_node(":cent2:" .. t.tier .. "_" .. v.name, {
				description = t.up .. " " .. MACHINE_NAME .. "\n\n" .. MACHINE_DESC,
				tiles = {
					"centrifuge_" .. t.tier .. "_top.png",  "centrifuge_" .. t.tier .. "_bottom.png",
					v.side, v.side,
					v.side, v.tile,
				},
