...
chatplus = chatplus or {}
chatplus.modpath = minetest.get_modpath("chatplus")
chatplus.players = chatplus.players or {}


function chatplus.on_joinplayer(player)
	local pname = player:get_player_name()
	chatplus.players[pname] = {
		last_pm_from = "",
	}
end

function chatplus.on_leaveplayer(player, timeout)
	local pname = player:get_player_name()
	chatplus.players[pname] = nil
end

if minetest.get_modpath("reload") then
  local c = "chatplus:core"
  local f = chatplus.modpath .. "/init.lua"
...
chatplus.nametag_color = color_nametag -- make public to other mods
...
chatplus.rewrite_message = function(chat2)
...
chatplus.send_all = function(from, prename, actname, postname, message, alwaysecho)
...
function chatplus.check_language(name, message)
...
local generate_coord_string = function(name)
  ...
  if command_tokens.mark.player_marked(name) then
    ...
    coord_string = " [" .. math.floor(pos.x) .. "," .. math.floor(pos.y) .. "," .. math.floor(pos.z) .. "]"
...
chatplus.on_chat_message = function(name, message)
	-- Trim input.
  ...
  if chatplus.check_language(name, message) then return end
  local coord_string = generate_coord_string(name)
  
  player_labels.on_chat_message(name, message)
  chatplus.send_all(name, "<", rename.gpn(name), coord_string .. "> ", message)
...
chatplus.handle_command_me = function(name, param)
  if not minetest.check_player_privs(name, {shout=true}) then
    minetest.chat_send_player(name, "# Server: You do not have 'shout' priv.")
  ...
  chatplus.send_all(name, "* <", rename.gpn(name), coord_string .. "> ", param, true)
...
chatplus.handle_command_msg = function(name, param)
  if not minetest.check_player_privs(name, {shout=true}) then
    minetest.chat_send_player(name, "# Server: You do not have 'shout' priv.")
		easyvend.sound_error(name)
    return -- Player doesn't have shout priv.
  ...
  if type(to)=="string" and type(newmsg)=="string" and string.len(newmsg) > 0 and string.len(to) > 0 then
		if gdac_invis.is_invisible(to) and to ~= name then -- If target is invisible, and player sending is not same as target ...
			if chatplus.players[name] and (chatplus.players[name].last_pm_from or "") ~= to then -- Do not permit, if player did not receive a PM from this target.
				if to == "MustTest" then
    ...
    if minetest.get_player_by_name(to) then
			-- Bad words in PMs.
      if chatplus.check_language(name, newmsg) then
				return
			end
			...
			minetest.after(0, function()
				...
				if chatplus.players[to] then
					chatplus.players[to].last_pm_from = name
				end
			end)
...
function chatplus.handle_command_r(name, param)
	local to = chatplus.players[name].last_pm_from or ""
	...
	return chatplus.handle_command_msg(name, to .. " " .. param) -- Prepend target name, and call normal /msg function.
end



if not chatplus.registered then
  minetest.register_chatcommand("me", {
    params = "<action>",
    description = "Send an 'action' message beginning with your name.",
    privs = {shout=true},
    func = function(name, param)
      chatplus.handle_command_me(name, chatplus.rewrite_message(param))
      return true
    end,
  })
  ...
  minetest.register_chatcommand("msg", {
    ...
    privs = {shout=true},
    func = function(name, param)
      chatplus.handle_command_msg(name, chatplus.rewrite_message(param))
      return true
    end,
  })
  ...
  minetest.register_chatcommand("r", {
    ...
    privs = {shout=true},
    func = function(name, param)
      chatplus.handle_command_r(name, chatplus.rewrite_message(param))
      return true
    end,
  })
  ...
  minetest.register_on_chat_message(function(name, message)
    chatplus.on_chat_message(name, chatplus.rewrite_message(message))
    return true -- Don't send message automatically, we already did this.
  end)

	minetest.register_on_joinplayer(function(...) return chatplus.on_joinplayer(...) end)
	minetest.register_on_leaveplayer(function(...) return chatplus.on_leaveplayer(...) end)
    
  chatplus.registered = true
end
