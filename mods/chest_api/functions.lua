...
function chest_api.protected_allow_metadata_inventory_move(pos, from_list, from_index, to_list, to_index, count, player)
	local meta = minetest.get_meta(pos)
	local name = minetest.get_node(pos).name
...
function chest_api.protected_allow_metadata_inventory_put(pos, listname, index, stack, player)
	local meta = minetest.get_meta(pos)
	local name = minetest.get_node(pos).name
	if not has_locked_chest_privilege(pos, name, meta, player) then
		return 0
...
function chest_api.protected_allow_metadata_inventory_take(pos, listname, index, stack, player)
	local meta = minetest.get_meta(pos)
	local name = minetest.get_node(pos).name
	if not has_locked_chest_privilege(pos, name, meta, player) then
		return 0
...
function chest_api.on_metadata_inventory_move(pos, from_list, from_index, to_list, to_index, count, player)
