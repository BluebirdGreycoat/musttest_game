...
function(stack)
	...
	if p1 then
		s1 = meta:get_string("p1")
	end
	if p2 then
		s2 = meta:get_string("p2")
	end
	if p3 then
		s3 = meta:get_string("p3")
	end
  ...
  if not user or not user:is_player() then return end
	...
	if pt.type == "node" then
		...
		if ctrl.sneak then
			...
			minetest.chat_send_player(pname, "# Server: WR-Config set secondary target! " .. s2)
		else
			local p1 = pt.under
			local s1 = minetest.pos_to_string(p1)
			meta:set_string("p1", s1)
			minetest.chat_send_player(pname, "# Server: WR-Config set primary target! " .. s1)
		end
	else
...
function(stack, user, pt)
  if not user or not user:is_player() then return end
	...
	if pt.type == "node" then
		...
		minetest.chat_send_player(pname, "# Server: WR-Config set tertiary target! " .. s3)
	end
	configurator.update_description(stack)
  return stack
end
