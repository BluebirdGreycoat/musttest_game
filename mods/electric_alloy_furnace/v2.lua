...
for j, t in ipairs({
	...
	func.formspec_active = function(fuel_percent, item_percent)
		...
		local formspec =
			...
			"list[context;src;" .. x1 .. ",0.5;2,1;]" ..
			"list[context;fuel;" .. x1+0.5 .. ",2.5;1,1;]" ..

			utility.progress_image(x1+0.5, 1.5, "machine_progress_bg.png", "machine_progress_fg.png", fuel_percent) ..
	...
	func.on_timer = function(pos, elapsed)
		...
		do
			if keeprunning then
				...
				local swapped = machines.swap_node(pos, "alloyf2:" .. t.tier .. "_active")
				...
				meta:set_int("fueltotaltime", 0)
				meta:set_float("srctime", 0)
				machines.swap_node(pos, "alloyf2:" .. t.tier .. "_inactive")
	...
	func.on_blast = function(pos)
		...
		drops[#drops+1] = "alloyf2:" .. t.tier .. "_inactive"
...
if not alloyf2.run_once then
	for j, t in ipairs({
		...
		for k, v in ipairs({
			...
			minetest.register_node(":alloyf2:" .. t.tier .. "_" .. v.name, {
				...
				tiles = {
					"alloyer_" .. t.tier .. "_top.png",  "alloyer_" .. t.tier .. "_bottom.png",
					"alloyer_" .. t.tier .. "_side.png", "alloyer_" .. t.tier .. "_side.png",
					"alloyer_" .. t.tier .. "_side.png", v.tile,
				},
	...
	local c = "alloyf2:core"
	local f = alloyf2.modpath .. "/v2.lua"
	reload.register_file(c, f, false)

	alloyf2.run_once = true
end
