...
local ORIGINAL_EDITOR_GUI = dofile(formspec.modpath .. "/editor_gui.lua")
...
local function make_editor(pname)
	...
	local offset = TEST_SIZE.x+TEST_PAD

	local NEWROOT = table.copy(ORIGINAL_EDITOR_GUI)
...
local function handle_logdump2(context, fields)
	if not fields.FormTabExportToLog then
		return
	end
	...
	local final = 'return ' .. dump(formtable, "  ")
	local header = "-- GUI definition table generated by Formspec Editor " .. formspec.MOD_VERSION

	core.log(('\n'):rep(5) .. ('-'):rep(80) .. '\n' .. header .. '\n' .. final .. '\n' .. ('-'):rep(80) .. ('\n'):rep(5))

	context:set_message("Exported GUI definitions to debug.txt.")
end
...
function formspec.on_player_receive_fields(player, formname, fields)
	...
	handle_logdump2(context, fields)
