...
inventory_plus.get_formspec = function(player,page)
  local get_buttons = function(ox,oy,mx) -- origin x, origin y, max x
    ...
    for _, i in ipairs(inventory_plus.buttons[player:get_player_name()]) do
      local x, y = ox + tb[b][1], oy + tb[b][2]
      formspec = formspec .. "image_button[" .. x .. "," .. y ..
        ";1,1;inventory_plus_" .. i .. ".png;" .. i .. ";]"
      b = b + 1
    end
  ...
  if page=="main" then
    ...
    --else
      return inventory_plus.inventory[player:get_player_name()]
        .. get_buttons(0,.5,1) ..
        "image[7,0.5;1,1;inventory_plus_trash.png]list[detached:trash;main;7,0.5;1,1;]"
    --end
  end
end
...
minetest.register_on_joinplayer(function(player)
	...
	minetest.after(1, function()
        -- Hot fix to avoid a crash when a player joins and leaves in the space of a second.
        local player2 = minetest.get_player_by_name(playername)
        if not player2 then return end
        if not player2:is_player() then return end
		inventory_plus.set_inventory_formspec(player2, inventory_plus.get_formspec(player2, inventory_plus.default))
	end)
end)
