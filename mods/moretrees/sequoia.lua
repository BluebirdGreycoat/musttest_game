...
minetest.register_node("moretrees:sequoia_sapling", {
    ...
    on_timer = function(pos, elapsed)
				if mtflower.can_grow(pos) then
					if mtflower.try_grow(pos, "moretrees:sequoia_tree", "moretrees:sequoia_leaves", "glowstone:minerals", "glowstone:minerals") then
						return
					end
				end
		...
		minetest.place_schematic(vector.add(vector.add(pos, {x=0, y=math_random(-2, 1), z=0}), SCHEMATIC_RELP), path, "random", nil, false)
		trunkgen.check_trunk(pos, 3, "moretrees:sequoia_tree")
		hb4.leafscatter.remove(pos, SCHEMATIC_MINP, SCHEMATIC_MAXP, "moretrees:sequoia_leaves", math_random(10, 30))
		hb4.leafscatter.add(pos, SCHEMATIC_MINP, SCHEMATIC_MAXP, "moretrees:sequoia_leaves", math_random(10, 30))
...
minetest.register_craft({
    ...
    recipe = {
        {"techcrafts:hinge_wood", "moretrees:sequoia_wood", "group:stick"},
        {"group:stick", "moretrees:sequoia_wood", "group:stick"}
    }
})
