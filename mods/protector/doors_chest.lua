...
function register_door(name, def)
	...
	minetest.register_node(name.."_b_1", {
		...
		on_rightclick = function(pos, node, clicker)
			if not minetest.test_protection(pos, clicker:get_player_name()) then
...
register_trapdoor("protector:trapdoor_steel", {
	description = S("Protected Iron Trapdoor"),
...
minetest.register_node("protector:chest", {
	...
	can_dig = function(pos,player)
		...
		if inv:is_empty("main") then
			...
			if not minetest.test_protection(pos, player:get_player_name()) then
				return true
			end
		end
	end,
