...
minetest.register_node("raspberries:fruit", {
  ...
  groups = {level = 1, seed = 1, snappy = 3, attached_node = 1, flammable = 2, foodrot = 1, notify_destruct = 1},
...
function(pos, oldnode, oldmetadata, digger)
  if digger and digger:is_player() then
    ...
    if string.find(wielditem:get_name(), "shovel") then
      ...
      local inv = digger:get_inventory()
      local leftover = inv:add_item("main", ItemStack("raspberries:fruit " .. math.random(1, 5)))
      minetest.add_item(pos, leftover)
