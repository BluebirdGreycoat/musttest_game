...
minetest.register_chatcommand("/p", {
	...
	func = function(name, param)
		...
		elseif param == "get" then --display current WorldEdit positions
			...
			worldedit.player_notify(name, "Unknown subcommand!")
		end
	end,
})
...
minetest.register_chatcommand("/stack2", {
	...
	func = function(name, param)
		...
		if repetitions == nil then
			worldedit.player_notify(name, "Invalid count!")
			return
		end
		repetitions = tonumber(repetitions)
		...
		if x == nil then
			worldedit.player_notify(name, "Invalid increments!")
			return
		end
...
minetest.register_chatcommand("/stretch", {
	...
	function(name, param)
		...
		if stretchx == 0 or stretchy == 0 or stretchz == 0 then
			worldedit.player_notify(name, "Invalid scaling factors!")
		end
		local count = check_region(name, param)
...
minetest.register_chatcommand("/allocate", {
	...
	func = function(name, param)
		...
		if not check_filename(param) then
			worldedit.player_notify(name, "Disallowed file name!")
			return
		end
...
minetest.register_chatcommand("/load", {
	...
	func = function(name, param)
		...
		if not string.find(param, "^[%w \t.,+-_=!@#$%%^&*()%[%]{};'\"]+$") then
			worldedit.player_notify(name, "Invalid file name!")
			return
		end
