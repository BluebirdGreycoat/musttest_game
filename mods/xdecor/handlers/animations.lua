...
-- Note: sit code is not used, due to being incompatible with other parts of the game code.
...
function xdecor.sit(pos, node, clicker, pointed_thing)
	...
	if default.player_attached[player_name] then
		...
		pova.set_physics_override(clicker, {speed=1, jump=1, gravity=1})
	...
	elseif not default.player_attached[player_name] and node.param2 <= 3 and
		...
		pova.set_physics_override(clicker, {speed=0, jump=0, gravity=0})
		clicker:set_pos(pos)
